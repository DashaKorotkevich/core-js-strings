...
function getStringLength(a) {
  if (typeof a === 'string') {
    return a.length; // Возвращаем длину строки
  }
  return 0; // Возвращаем 0, если a не строка
}
...
function removeLastOccurrences(str, value) {
  const substr = str.lastIndexOf(value);
  if (substr === -1) {
    return str;
  }
  return str.slice(0, substr).concat(str.slice(substr + value.length));
}
...
function countVowels(str) {
  ...
  return str.split('').reduce((count, char) => {
    return arrVowels.includes(char) ? count + 1 : count;
  }, 0);
}
...
function isPalindrome(str) {
  ...
  const firstHalf = newStr.slice(0, midIndex);
  const secondHalf = newStr
    .slice(newStr.length % 2 === 0 ? midIndex : midIndex + 1)
    .split('')
    .reverse()
    .join('');
...
function encodeToRot13(str) {
  ...
  str.split('').forEach((char) => {
    ...
    if (charCode >= 65 && charCode <= 90) {
      res += String.fromCharCode(((charCode - 65 + 13) % 26) + 65);
    } else if (charCode >= 97 && charCode <= 122) {
      res += String.fromCharCode(((charCode - 97 + 13) % 26) + 97);
    } else {
      res += char;
    }
  });
